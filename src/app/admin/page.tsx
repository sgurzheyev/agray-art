...
export default async function AdminHomePage() {
  ...
  return (
    <AdminFrame email={session.user!.email}>
      <p className="text-[11px] tracking-[0.28em] text-gold uppercase">Ателье</p>
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-5">
        {stats.map(([label, n]) => (
          <div key={label} className="border border-gold/20 p-4">
            ...
            <p className="mt-2 font-serif text-3xl text-gold">{n}</p>
          </div>
        ))}
      </div>
      <div className="mt-10 flex gap-3">
        <Link href="/admin/products/new" className="bg-gold px-5 py-2.5 text-xs tracking-[0.2em] text-ink uppercase">
          Новое изделие
        </Link>
        <Link href="/admin/buttons" className="border border-gold px-5 py-2.5 text-xs tracking-[0.2em] text-gold uppercase">
          Кнопки на сайте
        </Link>
      </div>
