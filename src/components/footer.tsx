  ...
  return (
    <footer className="mt-auto border-t border-gold/15 bg-ink">
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-3">
        <div>
          <p className="font-serif text-2xl tracking-[0.3em] text-gold">A.GRAY</p>
          <p className="mt-3 max-w-xs text-sm leading-relaxed text-muted">
            Ювелирный дом Андрея. Ручная работа, чёрный фон, золото. Москва · agray.art
          </p>
        </div>
        <div>
          ...
          <ul className="mt-4 grid grid-cols-2 gap-2 text-sm text-ivory/80">
            {categories.map((c) => (
              <li key={c.slug}>
                <Link href={`/catalog/${c.slug}`} className="hover:text-gold">
                  {c.name}
                </Link>
              </li>
            ))}
